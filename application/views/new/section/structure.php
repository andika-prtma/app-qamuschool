...
<html lang="en">
  ...
  <body>
  	<div class="container">
  		<?= form_open('calculate/structure_proses', ' id="form_soal"') ?>
  ...
  box-shadow: 10px 10px 5px #aaaaaa;">
		...
		<div class="" style="margin-top: 150px">		
			...
			<?php foreach ($soal->result() as $sl): ?> 
				...
				<?php
					if (intval($sl->nomor)<16){
						?>
							<label class="col-form-label"><?= $sl->soal ?></label>
						<?php
					}else{
						?>
							<img width="80%" src="<?= base_url() ?>assets/img/soal/<?= $sl->id_paket ?>/section2/<?= $sl->soal ?>" >
						<?php
					}
				?>
...
<script type="text/javascript">
    function simpan_jawaban(){
         $.ajax({
          url:"<?= base_url() ?>calculate/structure_proses",
          method:"POST",
